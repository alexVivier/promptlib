  ...
  return (
    <div className="flex items-center gap-1 flex-wrap flex-1">
      {activePrompt.tags.map((tag) => (
        <span
          key={tag}
          className="inline-flex items-center gap-0.5 px-2 py-0.5 bg-zinc-200 dark:bg-zinc-700 rounded-full text-xs"
        >
          {tag}
          <button
            onClick={() => removeTag(tag)}
            className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 ml-0.5"
          >
            &times;
          </button>
        </span>
